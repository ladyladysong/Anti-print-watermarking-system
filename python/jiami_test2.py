...
#计算m 嵌入水印信息的参数
def cal_K(img,char_range,wi,str1):
    global K
    count = 0   #所有的字符
    #NA,NB 字符图像个数
    black1 = [] #记录字符的黑色像素点
    ...
    for i in range(len(sum_img1)):
        img_one = sum_img1[i]
        change_all.append(embedInfo(img_one,m))
    ...
    for i in range(len(wi)):
        index = 0
        if wi[i] == '0':
            x,x1,y,y1 = char_range[index]
            #img_ = img[y:y1,x:x1]
            while ff:
                change,_ = changeA[index]
                if if_emded(img[y:y1,x:x1],change) and (flagA[index] == 0):

                    emdedding(img[y:y1,x:x1],change)
                    #embedInfo0(img_,m)
        ...
        if ff == False :
            break
            print change_sum
    #print "the positon with wi is: "
    print w_po
    #print "1cal_black(img)",cal_black(img) 
    #print "change_sum=",change_sum,"changeALL=",sum(changeALL)
    ...
    while change_sum > 0:
        if (index < len(sum_img1)):
            x,x1,y,y1 = char_range[index]
            change_sum = change_sum - embed_B(img[y:y1,x:x1], flag, change_sum)
            #print "change_sum -= change_sum:",change_sum
            #cv2.imwrite(dst_dir + "img" + str(index) + ".jpg",img[y:y1,x:x1] )
            index += 1
    #print "after adjust change_sum=",change_sum
    ...
    '''
    cv2.imwrite(dst_dir + str1 +"_new" + ".jpg", thresh2)
    return w_po



def embed_B(img1, flag, change):
    flip_b,flip_w,flip,flip1 = judge_flip(img1)
    #cv2.imshow('a',img1)
    #cv2.waitKey()
    count_ = 0
    #print "flip_b,flip_w=",flip_b,flip_w
    #需要调整黑色像素点为白色
    if ( flag == True ) :
        if change >= flip_b: 
            for i,j in flip:
...
def cutImage(thresh1):
    ...
    for i,line_range in enumerate(line_ranges): 
        ...
        for i1 in range(thresh1.shape[1]):
            if incol == 1 and w[i1] >= min_k:
                start1 = i1
                incol = 0
            elif (i1-start1 >= min_k) and w[i1] <= max_k and incol == 0:
                incol = 1
                l1 = start1 
                l2 = i1 +2
...
cal_K(thresh0,char_range, wi, str1)
